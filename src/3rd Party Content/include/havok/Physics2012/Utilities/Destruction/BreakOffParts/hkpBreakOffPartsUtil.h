...
HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,hkpBreakOffPartsListener);
	...
	virtual hkResult breakOffSubPart( const ContactImpulseLimitBreachedEvent& event, 
									  hkArray<hkpShapeKey>& keysBrokenOffOut, hkpPhysicsSystem& bodiesOut ) = 0;

		/// Breaks off a subpart and returns the newly created objects.
		/// This is called from deep within the engine, so the implementation has to be THREADSAFE!
		/// However there are never two calls to this listener in parallel thanks to a critical section in hkpBreakOffPartsUtil
	virtual hkResult breakOffSubPart( const ContactImpulseLimitBreachedEvent& event, 
									  hkpPhysicsSystem& bodiesOut ) { return HK_FAILURE; }
};
...
class hkpBreakOffPartsUtil: public hkpWorldExtension, public hkpContactImpulseLimitBreachedListener, protected hkpEntityListener, protected  hkpConstraintListener
{
	public:
...
#include <Physics2012/Utilities/Destruction/BreakOffParts/hkpBreakOffPartsUtil.inl>

...
protected:
		...
		HK_VIRTUAL_ON_PPU void extractSurfaceConstraintInfo(const hkpRootCdPoint& hit
			, hkSurfaceConstraintInfo& surfaceOut
		...
		void fireConstraintsProcessed( const HK_MANIFOLD_ARRAY_TYPE& manifold, hkSimplexSolverInput& input ) const;
		...
		hkReal moveToLinearCastHitPosition(const hkSimplexSolverOutput& output, const hkpRootCdPoint& hit, const hkpLinearCastInput& castInput, hkVector4& position);
...
#include <Physics2012/Utilities/CharacterControl/CharacterProxy/hkpCharacterProxy.inl>

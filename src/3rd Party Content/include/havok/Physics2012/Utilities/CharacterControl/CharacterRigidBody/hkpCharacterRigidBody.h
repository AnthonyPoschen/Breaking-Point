...
class hkpCharacterRigidBody : public hkReferencedObject, public hkpEntityListener, public hkpWorldPostSimulationListener
{
	public:
		...
		virtual void checkSupport( const hkStepInfo& stepInfo, hkpSurfaceInfo& ground ) const;	
			...
			/// A structure describing a supporting surface ,the contact point between it and the character and the
			/// surface velocity if a surface velocity modifier is present.
			/// The contact point normal always points towards the character. The surface velocity is in world space.
		struct SupportInfo
		{

...
class hkpCollideDebugUtil
{
	public:
		...
		static void HK_CALL drawObbLines(const hkTransform& t, const hkVector4& halfExtents, hkColor::Argb color, hkDebugDisplayHandler* displayHandler = HK_NULL, int id = 0, int tag = 0)
		{
				...
				for (int i = 0; i < 8; i++)
				{
					for ( int bit = 1; bit < 8; bit <<= 1 )
					{
						...
						if ( i < j )
						{
							...
							if(displayHandler)
							{
								displayHandler->displayLine(start, end, color, id, tag);
							}
							else
							{
								hkDebugDisplay::getInstance().displayLine(start, end, color, id, tag);
							}
						}
					}
				}
		...
		static void HK_CALL displayAabbs(hkArray<hkAabb>& aabbs, hkColor::Argb color, hkDebugDisplayHandler* displayHandler = HK_NULL, int id = 0, int tag = 0)
		{
#ifndef HK_DISABLE_DEBUG_DISPLAY

...
class hkpGravityGun : public hkpFirstPersonGun
{
	...
	public:
		...
		hkReal m_maxMassOfObjectPicked;               //+default(200.0) +hk.RangeReal(absmin=0.0,absmax=1000000.0) +hk.Ui(visible=true, label="Max. Object Mass")

		hkReal m_maxDistOfObjectPicked;				  //+default(50.0)	+hk.Semantics("DISTANCE") +hk.RangeReal(absmin=0.0,absmax=10000.0) +hk.Ui(visible=true, label="Max. Distance")

		hkReal m_impulseAppliedWhenObjectNotPicked;   //+default(100.0) +hk.RangeReal(absmin=0.0,absmax=1000.0) +hk.Ui(visible=true, label="Impulse on Unpicked")

		hkReal m_throwVelocity;						  //+default(50.0)	+hk.RangeReal(absmin=0.0,absmax=1000.0,softmin=0.0,softmax=100.0) +hk.Ui(visible=true, label="Throw Velocity")

...
class hkpShapeShrinker
{
	...
	HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,hkpShapeShrinker);
		...
		static hkpCylinderShape* HK_CALL shrinkCylinderShape( hkpCylinderShape* cylinderShape, hkReal relShrinkRadius, hkReal allowedDisplacement );
			...
			/// Note: \a optimize attempt to reduce the number of vertices produced by the shrinking process, but due to robustness issues, is disable by default.
		static hkpConvexVerticesShape* HK_CALL shrinkConvexVerticesShape( hkpConvexVerticesShape* convexShape, hkReal maximumConvexRadius, hkReal relShrinkRadius, hkReal allowedDisplacement, const char* shapeName = HK_NULL, hkBool optimize = false );
};

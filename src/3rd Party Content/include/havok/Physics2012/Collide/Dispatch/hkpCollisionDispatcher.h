...
#ifndef HKP_COLLISION_DISPATCHER_H
#define HKP_COLLISION_DISPATCHER_H
...
enum hkpCollisionDispatcherAgentType
{
	...
	///    and the other ignored (hkcdShapeType::ALL_SHAPE_TYPES).
	///    in this case we have to register a function with a pair of types ( HK_SHAPE_MY_TYPE, hkcdShapeType::ALL_SHAPE_TYPES )
	/// 
...
class hkpCollisionDispatcher : public hkReferencedObject
{
	...
	public:
		...
		~hkpCollisionDispatcher();
			...
			///   agent: dispatcher->registerAlternateShapeTypes( hkcdShapeType::TRIANGLE, hkcdShapeType::CONVEX );
		...
		inline hkBool32 hasAlternateType( hkpShapeType type, hkpShapeType alternateType );
			...
			/// Register an agent for a given pair of shape types, use hkcdShapeType::ALL_SHAPE_TYPES for one type if you want to register
	...
	public:
		...
		struct DebugEntry
		{
			...
			char m_typeB;
				...
				/// E.g. we have two spheres (m_typeAB = hkcdShapeType::SPHERE)
	...
	public:
		...
		enum CollisionQualityLevel
		{
			...
			COLLISION_QUALITY_PSI,
				...
				/// This only back steps the debris objects.
...
#include <Physics2012/Collide/Dispatch/hkpCollisionDispatcher.inl>

#endif // HKP_COLLISION_DISPATCHER_H

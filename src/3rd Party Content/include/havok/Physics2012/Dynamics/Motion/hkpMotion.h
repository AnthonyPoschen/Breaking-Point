...
public:
	...
	/// Velocity and inverse virtual mass of point p along the supplied normal.
	virtual void getProjectedPointVelocitySimd(const hkVector4& p, const hkVector4& normal, hkSimdReal& velOut, hkSimdReal& invVirtMassOut) const = 0;
...
public:
	...
	hkpMotion( class hkFinishLoadedObjectFlag flag ) : hkReferencedObject(flag) { if ( flag.m_finishing ) { m_gravityFactor.setOne(); } }
	...
	inline void zeroNumInactiveFramesMt(int select, int worldDeactivationNumInactiveFramesSelectFlag);
		...
		// Set the deactivation variables such that the motion will be treated as inactive by the next check.
...
#include <Physics2012/Dynamics/Motion/hkpMotion.inl>

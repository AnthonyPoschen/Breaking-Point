...
{
	return m_inertiaAndMassInv.getComponent<3>();
}
...
void hkpMotion::getPointVelocity(const hkVector4& p, hkVector4& pointVelOut) const
{
	...
	hkVector4 relPos; relPos.setSub( p, getCenterOfMassInWorld() );
	pointVelOut.setCross( m_angularVelocity, relPos);
	pointVelOut.add( m_linearVelocity );
}
...
{
	m_motionState.m_linearDamping.setReal<true>(d);
}


void hkpMotion::setAngularDamping( hkReal d )
{
	m_motionState.m_angularDamping.setReal<true>(d);
}

void hkpMotion::setTimeFactor( hkReal f )
{
	m_motionState.m_timeFactor.setReal<true>(f);
}
...
{
	return m_gravityFactor.getReal();
}

void hkpMotion::setGravityFactor( hkReal gravityFactor )
{
	m_gravityFactor.setReal<true>(gravityFactor);
}

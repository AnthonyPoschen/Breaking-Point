...
namespace hkWorldOperation
{
	...
	{
		public:
			HK_DECLARE_CLASS_ALLOCATOR(hkWorldOperation::UserCallback);
...
#		endif
	...
	struct SetRigidBodyLinearVelocity : public BaseOperation
	{
		...
		hkpRigidBody* m_rigidBody;
		HK_ALIGN(hkReal m_linearVelocity[3],HK_REAL_SIZE);
	};
	struct SetRigidBodyAngularVelocity : public BaseOperation
	{
		...
		hkpRigidBody* m_rigidBody;
		HK_ALIGN(hkReal m_angularVelocity[3],HK_REAL_SIZE);
	};
	struct ApplyRigidBodyLinearImpulse : public BaseOperation
	{
		...
		hkpRigidBody* m_rigidBody;
		HK_ALIGN(hkReal m_linearImpulse[3],HK_REAL_SIZE);
	};
	...
	struct ApplyRigidBodyAngularImpulse : public BaseOperation
	{
		...
		hkpRigidBody* m_rigidBody;
		HK_ALIGN(hkReal m_angularImpulse[3],HK_REAL_SIZE);
	};
	...
	struct RequestDeactivateEntity : public BaseOperation 
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_DYNAMICS, RequestDeactivateEntity );

		RequestDeactivateEntity() : BaseOperation(REQUEST_DEACTIVATE_ENTITY) {}
		hkpEntity* m_entity;
	};
	...
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_DYNAMICS, ConstraintCollisionFilterConstraintBroken );
...
#include <Physics2012/Dynamics/World/Util/hkpWorldOperationQueue.inl>

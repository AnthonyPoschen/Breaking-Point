...
inline void hkpContactPointEvent::setNormal( const hkVector4& normal )
{
	// Set normal and estimated separating distance
	const hkSimdReal estimatedDistance = m_contactPoint->getDistanceSimdReal() * m_contactPoint->getNormal().dot<3>( normal );
	m_contactPoint->setSeparatingNormal( normal, estimatedDistance );

	// Set separating velocity if necessary
	if( m_separatingVelocity )
	{
		*m_separatingVelocity = hkpSimpleContactConstraintUtil::calculateSeparatingVelocity( m_bodies[0], m_bodies[1], m_bodies[0]->getCenterOfMassInWorld(), m_bodies[1]->getCenterOfMassInWorld(), m_contactPoint ).getReal();
	}
}
...
inline hkReal hkpContactPointEvent::getSeparatingVelocity() const
{
	...
	{
		return hkpSimpleContactConstraintUtil::calculateSeparatingVelocity( m_bodies[0], m_bodies[1], m_bodies[0]->getCenterOfMassInWorld(), m_bodies[1]->getCenterOfMassInWorld(), m_contactPoint ).getReal();
	}
}

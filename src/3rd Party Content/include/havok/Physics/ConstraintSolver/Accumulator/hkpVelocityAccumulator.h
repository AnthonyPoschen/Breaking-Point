...
class hkpVelocityAccumulator 
{
	public:
		...
		hkEnum<hkpAccumulatorType,hkUint8> m_type;
			...
			/// Internal debugging flag to prevent enforce to correctly read the 'scratch' variables
	...
	public:
		...
		HK_FORCE_INLINE void setFixed()
		{
			...
			m_gravityFactor = hkReal(0);
			this->m_invMasses.setZero();
			this->m_linearVel.setZero();
			this->m_angularVel.setZero();
			this->m_scratch3.setZero();	// setCenterOfMassInWorld(hkVector4::getConstant<HK_QUADREAL_0>());
...
///
class hkpVelocityAccumulator2 : public hkpVelocityAccumulator
{
	public:

		hkQuaternion m_coreQworld;

		/// The linear velocity in world space which was used to integrate this motion.
		hkVector4 m_prevIntegrationLinearVelocity;

		/// The angular velocity in local space which was used to integrate this motion.
		hkVector4 m_prevIntegrationAngularVelocity;
};

...
///
/// Notes:
///       - Internally, the convex hull stores two sets of coordinates for each input point,
...
class hkgpConvexHull : public hkReferencedObject
{
	public:
		...
		struct BuildConfig
		{
			...
			hkBool m_alwaysComputeProjectionPlane;		///< If true, attempt to compute a projection plane for 3d convex hull (default: false).
			...
			hkBool m_sortInputs;						///< Sort inputs to improve performance (in most cases, pathological cases like cylinders can become slower) (default: true).
		...
		struct AbsoluteScaleConfig 
		{
			enum Method
			{
				SKM_VERTICES,			///< Scale by shifting vertices with respect to the center of mass.
				SKM_PLANES,				///< Scale by shifting planes, without considering special cases.
				SKM_CLAMPED_PLANES,		///< Scale by shifting planes, with extra checks to limit shift distances.
				NUM_METHODS
			};
			...
			/// The feature preservation factor, between 0.0f and 1.0f.
			/// If set to 0.0f, vertex displacement will not be considered. Sharp corners and edges may move by a significant distance.
			/// If set to 1.0f, each vertex will move no more than the scale distance. Sharp corners and edges will be preserved.
			/// (default: 0.0f).
		...
		struct IUserObject
		{
			...
			virtual void append( const IUserObject* other )	{ HK_ERROR(0x5D0314E3, "hkgpConvexHull::IUserObject::append method not implemented"); }
		};
	...
	public:
		...
		hkResult			simplify(const SimplifyConfig& config);
		...
		/// Absolute scaling of convex hull, since scaling (especially shrinking) is limited, this function returns the actual applied scale.
		hkReal				absoluteScale(const hkReal offset,const AbsoluteScaleConfig& config=AbsoluteScaleConfig());
		...
		hkSimdReal			maximumDistanceToPlanes(hkVector4Parameter point, int* planeIndex=HK_NULL) const;
		...
		hkSimdReal			getWidth(hkVector4Parameter direction) const;

		/// Gets the minimum width of the convex hull.
		hkSimdReal			getMinWidth() const;

		/// Gets the minimum width of the convex hull.
		hkSimdReal			getMaxWidth() const;

		/// Gets minimum and maximum widths of the convex hull.
		void				getWidthBounds(hkSimdReal& minWidth,hkSimdReal& maxWidth) const;

		/// Gets the minimum distance from a plane.
		hkSimdReal			getMinimumDistanceFromPlane(hkVector4Parameter plane) const;

		/// Gets the maximum distance from a plane
		hkSimdReal			getMaximumDistanceFromPlane(hkVector4Parameter plane) const;
		...
		/// Compute the local transform that align inertia and COM.
		void				computeLocalTransform(hkTransform& localTransform) const;
		...
		/// So to get the edges for vertex v: \code { int* pe = &edgesOut[ vertexOffsetsOut[v]; int numEdges = pe[0]; for ( int ei = 0; ei<numEdges; ei++ ){ printf( "edge %e", pe[ei+1] }; } \endcode
		void				generateVertexAdjacencyGraph( hkArray<int>& edgesOut, hkArray<int>& vertexOffsetsOut) const;

		/// For each vertex find all touching planes. 
		/// Output: plane i of vertex v: \code numPlanes = startPlaneOffsetOut[v+1] - startPlaneOffsetOut[v];  plane[i] = getPlane( planeIndexOut[ startPlaneOffsetOut[v] + i ]; \endcode
		void				generatePlanesPerVertex( hkArray<int>& startPlaneOffsetOut, hkArray<int>& planeIndicesOut ) const;

		/// Generates hkGeometry.
		void				generateGeometry(Inputs inputs, struct hkGeometry& geometryOut, int material = -1, bool planarTwoSided = true) const;
		...
		/// Compute the approximate concavity of the set of points with respect to a given convex hull.
		static hkSimdReal HK_CALL		computeConcavityFromPoints(const hkgpConvexHull* hull, const hkArray<hkVector4>& points);

		/// Compute the approximate concavity of a geometry with respect to a given convex hull.
		static hkSimdReal HK_CALL		computeConcavityFromGeometry(const hkgpConvexHull* hull, const hkGeometry& geometry);
		...
		static hkSimdReal HK_CALL		findBestUnion(const hkgpConvexHull* a, const hkgpConvexHull* b, const IBooleanFunction& f, hkgpConvexHull*& aUnb, hkgpConvexHull*& pb);
		...
		/// Create a planar slice from a 3d convex hull.
		static hkBool HK_CALL			createPlanarSlice(const hkgpConvexHull* domain, hkVector4Parameter plane, hkgpConvexHull*& sliceOut);

		/// Reset the unique identifier generator to zero.
		/// Use for debugging purposes only.
		static void HK_CALL				resetUidGenerator();

	private:

		inline hkgpConvexHull( const hkgpConvexHull& other ) : hkReferencedObject(other) {}

		void operator=( const hkgpConvexHull& other ) {}

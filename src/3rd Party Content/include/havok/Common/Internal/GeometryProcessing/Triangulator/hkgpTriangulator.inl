...
inline int												TRIANGULATOR_TYP::getOpenBoundaries(hkArray<Edge>* edges) const
{
	...
	/* Partition					*/ 
	const_cast<TRIANGULATOR_TYP*>(this)->partition();
...
HKGP_INLINE void										TRIANGULATOR_TYP::reset(bool createDomainTriangulation)
{
	...
	if(createDomainTriangulation)
	{
		...
		if(cfgIsCCW)
		{
			t0 = newTriangle(c00,c01,c11);
			t1 = newTriangle(c00,c11,c10);

		}
		else
		{
			t0 = newTriangle(c00,c10,c11);
			t1 = newTriangle(c00,c11,c01);
		}
...
//
TRIANGULATOR_HDR
inline hkResult												TRIANGULATOR_TYP::clone(hkgpTriangulatorType* other) const
{
	other->reset(false);
	...
	for(Triangle* t=m_mesh.m_triangles.getFirst();t;t=t->next())
	{
		...
		{
			hkResult tres = HK_FAILURE;
			tmap.tryInsert(t,nt, tres);
			if (tres != HK_SUCCESS)
			{
				return HK_FAILURE;
			}
		...
		for(int i=0;i<3;++i)
		{
			...
			else
			{
				...
				{
					hkResult eres;
					emap.tryInsert(te.uid(),ne.uid(), eres);
...
inline void												TRIANGULATOR_TYP::setDomain(const hkVector4& dmin,const hkVector4& dmax,hkReal angle,bool margin)
{
	...
	m_margin	=	margin?1:0;
	hkSimdReal	invExt; invExt.setReciprocal<HK_ACC_FULL,HK_DIV_IGNORE>(hkSimdReal::fromInt32(cfgBoundary-m_margin*2));
	hkVector4		scl;
	hkSimdReal s,c;
	hkVector4Util::sinCos(hkSimdReal::fromFloat(angle),s,c);
	scl.setSub(dmax,dmin);
	scl.mul(invExt);

	hkSimdReal zero; zero.setZero();
	m_localToWorld.setIdentity();

	m_localToWorld.getColumn(0).set(c*scl.getComponent<0>(),-s*scl.getComponent<0>(),zero,zero);
	m_localToWorld.getColumn(1).set(s*scl.getComponent<1>(),c*scl.getComponent<1>(),zero,zero);
	hkVector4& column = m_localToWorld.getColumn(3);
    column.setSelect<hkVector4ComparisonMask::MASK_XY>(dmin,hkVector4::getConstant<HK_QUADREAL_0001>());

	hkMatrix4Util::setInverse(m_localToWorld,m_worldToLocal,zero);
	reset();
}
//
TRIANGULATOR_HDR
inline void												TRIANGULATOR_TYP::setDomainFromPoints(const hkStridedVertices& points, const hkMatrix4& worldToXYPlane, bool margin, bool keepRatio)
{
	...
	for(int i=0,ni=points.getSize();i<ni;++i)
	{
		hkVector4	tp;
		hkVector4	vt;
		vt.load<3,HK_IO_NATIVE_ALIGNED>(hkAddByteOffsetConst(points.m_vertices, i*points.m_striding));
		vt.zeroComponent<3>(); // avoid calc with NaNs
	...
	/* Fixup zero bounds				*/ 
	{
		const hkSimdReal minExtend = hkSimdReal_Eps * hkSimdReal::fromFloat(100.0f);
	...
	/* Keep ratio						*/ 
	hkVector4		extents;extents.setSub(dmax,dmin);
	if(keepRatio)
	{
		if (extents.getComponent<0>().isGreater(extents.getComponent<1>()))
		{
			const hkSimdReal	half = (extents.getComponent<0>()-extents.getComponent<1>())*hkSimdReal_Half;
			dmin.setComponent<1>(dmin.getComponent<1>() - half);
			dmax.setComponent<1>(dmax.getComponent<1>() + half);
			extents.broadcast<0>();
		}
		else
		{
			const hkSimdReal	half = (extents.getComponent<1>()-extents.getComponent<0>())*hkSimdReal_Half;
			dmin.setComponent<0>(dmin.getComponent<0>() - half);
			dmax.setComponent<0>(dmax.getComponent<0>() + half);
			extents.broadcast<1>();
		}
	}
	/* Evaluate transforms				*/ 
	...
	trs.setDiagonal(ext);
	hkVector4 offDiag;
	offDiag.setAll(hkSimdReal::fromInt32(m_margin));
	offDiag.subMul(dmin,ext);
	hkVector4 col3;
	col3.setSelect<hkVector4ComparisonMask::MASK_XY>(offDiag,trs.getColumn<3>());
	trs.setColumn<3>(col3);
	m_worldToLocal.setMul(trs,worldToXYPlane);
	hkMatrix4Util::setInverse(m_worldToLocal,m_localToWorld,hkSimdReal_0);
	reset();
}
//
TRIANGULATOR_HDR
...
{
	hkMatrix4 mat; mat.setIdentity();
	if(useMainAxis)
	{
		int			majorAxis = planeNormal.getIndexOfMaxAbsComponent<3>();
		hkVector4 t = mat.getColumn<2>(); mat.setColumn<2>(mat.getColumn(majorAxis)); mat.setColumn(majorAxis,t);
		if(majorAxis<2 && planeNormal.getComponent(majorAxis).isGreaterZero())
		{
			hkVector4	c0; c0.setNeg<4>(mat.getColumn<0>());
			hkVector4	c1; c1.setNeg<4>(mat.getColumn<1>());
			mat.setColumn<0>(c0);
			mat.setColumn<1>(c1);
		}
		else if(majorAxis==2 && planeNormal.getComponent(majorAxis).isLessZero())
		{
			hkVector4	c0; c0.setNeg<4>(mat.getColumn<0>());
			mat.setColumn<0>(c0);
		}
	...
	{
		const hkVector4& xyPlane = hkVector4::getConstant<HK_QUADREAL_0010>();
		hkQuaternion rot;	hkQuaternionUtil::_computeShortestRotation(planeNormal, xyPlane, rot);
		hkTransform trs;
		trs.setRotation(rot);
		trs.setTranslation(hkVector4::getZero());
		mat.set(trs);
	}
...
//
TRIANGULATOR_HDR
inline void										TRIANGULATOR_TYP::getEffectiveAccuracy(hkVector4& acc) const
{
	hkVector4 td0; m_localToWorld.transformDirection(hkVector4::getConstant<HK_QUADREAL_1000>(),td0); 
	const hkSimdReal acc0 = td0.length<3>();

	hkVector4 td1; m_localToWorld.transformDirection(hkVector4::getConstant<HK_QUADREAL_0100>(),td1); 
	const hkSimdReal acc1 = td1.length<3>();
	
	acc.set(acc0,acc1,hkSimdReal_0,hkSimdReal_0);
}
...
HKGP_FORCE_INLINE void								TRIANGULATOR_TYP::transform(const Vertex* v, hkVector4& result) const
{
	...
	vReal.zeroComponent<3>();
	m_localToWorld.transformPosition(vReal, result);
}

TRIANGULATOR_HDR
HKGP_FORCE_INLINE void								TRIANGULATOR_TYP::transform(const Triangle* t, hkVector4 vertices[3]) const
{
	hkVector4 v0; transform(t->vertex(0),v0);
	hkVector4 v1; transform(t->vertex(1),v1);
	hkVector4 v2; transform(t->vertex(2),v2);
...
//
TRIANGULATOR_HDR
HKGP_FORCE_INLINE void								TRIANGULATOR_TYP::transformUnquantized(hkVector4Parameter v, hkVector4& x) const
{
	m_worldToLocal.transformPosition(v,x);
}	
//
TRIANGULATOR_HDR
HKGP_FORCE_INLINE void								TRIANGULATOR_TYP::quantize(hkVector4Parameter v, typename TRIANGULATOR_TYP::Vertex& result) const
{
	...
	const int	bmx=cfgBoundary-m_margin;
	{
		...
		result.m_x = hkMath::clamp((int)(roundedInt.getU32<0>()),bmi,bmx);
		result.m_y = hkMath::clamp((int)(roundedInt.getU32<1>()),bmi,bmx);
	}
}
//
TRIANGULATOR_HDR
HKGP_FORCE_INLINE void 									TRIANGULATOR_TYP::transform(hkVector4Parameter v, typename TRIANGULATOR_TYP::Vertex& result) const
{
	hkVector4 x;
	transformUnquantized(v,x);
	quantize(x, result);
}	
//
TRIANGULATOR_HDR
...
HKGP_FORCE_INLINE typename TRIANGULATOR_TYP::Edge			TRIANGULATOR_TYP::conformEdge(const Edge& edge)
{
	...
	hkBool32 isNaked = edge.isNaked();

	const int expandSize = isNaked ? 2 : 4;
	hkResult expandRes = m_stack.reserve( m_stack.getSize() + expandSize );
	if(expandRes == HK_FAILURE)
	{
		m_status = STATUS_OUT_OF_MEMORY;
		return Edge::null();
	}

	Edge* stackEnd = m_stack.expandByUnchecked(expandSize);
	stackEnd[0].setNext(edge);
	stackEnd[1].setPrev(edge);
	
	if(!isNaked)
	{
		stackEnd[2].setNextCcw(edge);
		stackEnd[3].setNextCw(edge);
	}
...
HKGP_FORCE_INLINE typename TRIANGULATOR_TYP::Location		TRIANGULATOR_TYP::locateVertex(const Edge& rootArg,int x,int y) const
{
	...
	Edge	root = rootArg;
	bool	useRootArg = true;
	for(Triangle* base=m_mesh.m_triangles.getFirst(); base; useRootArg=false)
	{
		tExtInt	last=0;
		
		if(!useRootArg)
		{
			// The first time we use 'rootArg' to search for the vertex, but after we just try all triangles in order (slow but rare).
			root=Edge(base,0);
			base=base->next();
		}
...
inline hkBool32							TRIANGULATOR_TYP::fetchFanBoundary(Edge edge,T& boundaries)
{
	if(edge.start()->isBoundary())
	{
		...
		do
		{
			...
			b.m_constrained	=	edge.next().isConstrained();
			last			=	edge;
			edge			=	edge.prev().link();
		} while(edge.isValid());
	...
	case	Location::IN_TRIANGLE:
		{
			...
			if(conform)
			{
				hkResult expandRes = m_stack.reserve( m_stack.getSize() + 3 );
				if(expandRes == HK_FAILURE)
				{
					m_status = STATUS_OUT_OF_MEMORY;
					return Insertion(Insertion::OUT_OF_MEMORY,Edge::null());
				}

				Edge* stackEnd = m_stack.expandByUnchecked(3);
				stackEnd[0] = Edge(t0,0);
				stackEnd[1] = Edge(t1,0);
				stackEnd[2] = edge;
				flushConform();
	...
	case	Location::ON_EDGE:
		if(edgeSplit || !edge.isConstrained())
		{
			...
			if(edge.isBound())
			{
				...
				if(conform)
				{
					hkResult expandRes = m_stack.reserve( m_stack.getSize() + 4 );
					if(expandRes == HK_FAILURE)
					{
						m_status = STATUS_OUT_OF_MEMORY;
						return Insertion(Insertion::OUT_OF_MEMORY,Edge::null());
					...
					Edge* stackEnd = m_stack.expandByUnchecked(4);
					stackEnd[0].setNext(edge);
					stackEnd[1].setNext(link);
					stackEnd[2] = Edge(t0,0);
					stackEnd[3] = Edge(t1,0);
				}
			}
			else
			{
				...
				if(conform)
				{
					hkResult expandRes = m_stack.reserve( m_stack.getSize() + 2 );
					if(expandRes == HK_FAILURE)
					{
						m_status = STATUS_OUT_OF_MEMORY;
						return Insertion(Insertion::OUT_OF_MEMORY,Edge::null());
					...
					Edge* stackEnd = m_stack.expandByUnchecked(2);
					stackEnd[0].setNext(edge);
					stackEnd[1] = Edge(t0,0);
				}
			}
			if(conform)
			{
...
//
#define HKGP_TRIANGULATOR_ENABLE_ERROR_RECOVERY 1
...
HK_ON_CPU(inline) typename TRIANGULATOR_TYP::Insertion	TRIANGULATOR_TYP::insertCrossingEdge(int x0,int y0,int x1,int y1,bool conform, const IEDGECROSSING& icrossing, const EdgeData* data)
{
	...
	for(;;)
	{
		...
		#if !HKGP_TRIANGULATOR_ENABLE_ERROR_RECOVERY
		x0org = x0;
		y0org = y0;
		#endif
		...
		case	Insertion::CROSSING_CONSTRAINED_EDGE:
			{
				...
				const tExtInt	num		=	x3 * (tExtInt) (y0org - y2) + x0org * (tExtInt) (y2 - y3) + x2 * (tExtInt)(y3 - y0org);
				const tExtInt	den		=	(x3 - x2) * (tExtInt) (y0org - y1) + (x0org - x1) * (tExtInt) (y2 - y3);
				const hkReal	sol		=	den ? num/(hkReal)den : 0;
				const hkReal	ifx		=	x0org + (x1 - x0org) * sol;
				const hkReal	ify		=	y0org + (y1 - y0org) * sol;
				hkInt32			ix		=	hkMath::hkToIntFast(ifx);
				hkInt32			iy		=	hkMath::hkToIntFast(ify);
				...
				if(den)
				{
					...
					if(!split)
					{
						...
						hkSimdReal	nearest;
						nearest.setMin( isecPoint.distanceToSquared(endPoints[0]), isecPoint.distanceToSquared(endPoints[1]) );

						for(int i=1,ni=hkMath::min2((hkInt32)cfgSearchSteps,(hkInt32)(4+hkMath::max2(absDx,absDy)*8));i<ni;++i)
						{

							const hkSimdReal fi = hkSimdReal::fromInt32(i);
							const hkSimdReal fni = hkSimdReal::fromInt32(ni);
							hkSimdReal sf; sf.setDiv(fi,fni);
							for(int s=0;s<2;++s)
							{
								hkVector4	isec;
								isec.setInterpolate(isecPoint,endPoints[s],sf);
								const hkSimdReal sqd=isec.distanceToSquared(isecPoint);
								if(!split && sqd<nearest)
								{
									hkInt32		tx,ty;
									(isec.getComponent<0>()+hkSimdReal_Half).storeSaturateInt32(&tx);
									(isec.getComponent<1>()+hkSimdReal_Half).storeSaturateInt32(&ty);
...
// Try to reserve one more. If the allocation failed, return.
#define CHECK_ARRAY_SIZE_AND_RETURN_IF_OUT_OF_MEM(_arrayName, _returnval) { hkResult _res = (_arrayName).reserve((_arrayName).getSize() + 1); if (_res != HK_SUCCESS) return _returnval; }
...
// 				{
// 					draw();
// 					debugRenderNow().setTitle("partition step");
// 				}
				for(int i=0;i<3;++i)
				{
					...
					if(ne.isValid()&&(nt->m_region==nullregion))
					{
						if(ne.isConstrained())
						{
							// Should never happen on SPU. If it does it means we have miscalculated the max number of edges in our initial allocate
...
TRIANGULATOR_HDR
template <typename FLOODPOLICY>
inline hkResult												TRIANGULATOR_TYP::partitionWithPolicy(const FLOODPOLICY& policy)
{
	/* Reset regions*/ 
	for(Triangle* t=m_mesh.m_triangles.getFirst();t;t=t->next())
	{
...
// 				{
// 					draw();
// 					debugRenderNow().setTitle("partitionWithPolicy step");
// 				}
				for(int i=0;i<3;++i)
				{
					Edge		ne=Edge(t,i).link();
					...
					if(ne.isValid()&&(nt->m_region==nullregion))
					{
						hkBool32 constrained = ne.isConstrained() | ne.link().isConstrained();
						const EdgeData*	d = getEdgeData(ne);
						const EdgeData*	dl = getEdgeData(ne.link());
						tSets insideCount = 0;
						insideCount += d ? policy.getSetsFromEdgeData(d): 0; 
						insideCount += dl ? policy.getSetsFromEdgeData(dl): 0; 
						if(constrained && (insideCount % 2))
						{
							// Should never happen on SPU. If it does it means we have miscalculated the max number of edges in our initial allocate
							CHECK_ARRAY_SIZE_AND_RETURN_IF_OUT_OF_MEM(*next, HK_FAILURE)
								next->pushBack(ne);
						}
						else
						{
							// Should never happen on SPU. If it does it means we have miscalculated the max number of edges in our initial allocate
							CHECK_ARRAY_SIZE_AND_RETURN_IF_OUT_OF_MEM(*current, HK_FAILURE)
								current->pushBack(ne);
						}
					}
				}
...
{
	// Keep track of the stack size in parallel with m_stack.
	// This avoids load-hit-stores from checking getSize() right after we push/pop.
	int			stackSize = m_stack.getSize();
	unsigned	maxPasses = 1+m_mesh.m_triangles.getSize()*3;
	do
	{
		hkBool32 lastEdgeIsConform = isConform( m_stack.back() );
		if(!lastEdgeIsConform)
		{
			...
			if(edge.canFlip())
			{
				...
				stackEnd[0].setNext(edge);// = edge.next().uid();
				stackEnd[1].setNextCw(edge);// = edge.nextCw().uid();
			}
		}
		else
		{
		...
		HK_ASSERT(0x1e14d697, stackSize == m_stack.getSize() );
	} while(stackSize>0 && (--maxPasses)>0);
	if(maxPasses==0)
	{

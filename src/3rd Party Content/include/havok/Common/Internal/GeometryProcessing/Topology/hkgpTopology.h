...
HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,hkgpTopology);
	...
	template <typename T>
	static HKGP_FORCE_INLINE void		ulong2uid(hkUlong uid, T*& t,hkUint32& i)	{ i = (hkUint32)(uid&3);uid=hkClearBits(uid,3);t=*(T**)&uid; }
	...
	public:
		...
		/// set this to edge.next()
		HKGP_FORCE_INLINE void			setNext(const EDGE& edge)		{ m_triangle = edge.m_triangle; m_index = ip1mod3(edge.m_index); }
		...
		/// set this to edge.prev()
		HKGP_FORCE_INLINE void			setPrev(const EDGE& edge)		{ m_triangle = edge.m_triangle; m_index = ip2mod3(edge.m_index); }

		/// return the previous edge along current edge triangle (CCW)
		HKGP_FORCE_INLINE void			prevInPlace()					{ m_index = ip2mod3(m_index); }
		
		/// return the edge bound with this edge if any
		HKGP_FORCE_INLINE EDGE			link() const					{ return(EDGE(m_triangle->link(m_index))); }

		/// sets this to edge.link()
		HKGP_FORCE_INLINE void			setLink(const EDGE& edge)		{ unpackUid( edge.m_triangle->link(edge.m_index), m_triangle, m_index ); }
		...
		// set this to edge.nextCw()
		HKGP_FORCE_INLINE void			setNextCw(const EDGE& edge)
		{
			hkUint32 linkIndex;
			unpackUid( edge.m_triangle->link(edge.m_index), m_triangle, linkIndex );
			m_index = ip2mod3(linkIndex);
		}
		
		/// next edge ccw in the dual mesh
		HKGP_FORCE_INLINE EDGE			nextCcw() const					{ return(link().next()); }

		/// set this to edge.nextCcw()
		HKGP_FORCE_INLINE void			setNextCcw(const EDGE& edge)
		{
			hkUint32 linkIndex;
			unpackUid( edge.m_triangle->link(edge.m_index), m_triangle, linkIndex );
			m_index = ip1mod3(linkIndex);
		}

...
namespace hkBufferCompression
{
		...
		/// byte written.
	hkResult HK_CALL decompressBufferChecked(const void* in, hk_size_t inlen, void*& out, hk_size_t outlen);
	...
	void HK_CALL decompressBufferFast(const void* in, hk_size_t inlen, void*& out);
	...
	inline hk_size_t HK_CALL compressBuffer(const void* in, hk_size_t inlen, void* out, hk_size_t outlen, hkBool fastest = true);

		/// Compress using a hashing tree. This gives good compression at reasonable speed.
	hk_size_t HK_CALL hashCompress(const void* in, hk_size_t inlen, void* outbuf, hk_size_t outlen);

		/// Compress using a suffix tree. This gives optimal compression in exchange for
		/// memory and speed.
	hk_size_t HK_CALL suffixTreeCompress(const void* data, hk_size_t length, void* outbuf, hk_size_t outlength);
}

...
class hk1AxisSweep
{
	public:
		...
		struct AabbInt : public hkAabbUint32
		{
			...
			const hkUint32& getUserData() const
			{
				// Simply returning m_max[3] here causes a compiler error when static code analysis is enabled as m_max only has 3 components.
				// minP[3] points to m_max's alignment padding, which is where the AabbInt's userdata is stored.
		...
		static void HK_CALL collide( const AabbInt* pa, int numA, const AabbInt* pb, int numB, hkArray<hkKeyPair>& pairsOut);
		...
		static void HK_CALL collide(const hk1AxisSweep::AabbInt* pa, int numA, hkArray<hkKeyPair>& pairsOut);
		...
		static void HK_CALL sortAabbs(hk1AxisSweep::AabbInt* aabbs, int size, hkArrayBase<hkRadixSort::SortData32>& sortArray, hkArrayBase<hk1AxisSweep::AabbInt>& sortedAabbs );

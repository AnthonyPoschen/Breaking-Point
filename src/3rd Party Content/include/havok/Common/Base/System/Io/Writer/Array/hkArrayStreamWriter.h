...
class hkArrayStreamWriter : public hkStreamWriter
{
	...
	HK_DECLARE_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE);
		...
		hkArrayStreamWriter(hkArray<char>* arr, ArrayOwnership o)
			: m_arr(arr), m_allocator(hkArray<char>::AllocatorType().get(arr)), m_offset(arr->getSize()), m_ownerShip(o)
		{
			nullTerminate();
		}

			/// Generic Constructor independent from the allocator used
		hkArrayStreamWriter(hkArrayBase<char>* arr, hkMemoryAllocator& allocator, ArrayOwnership o)
			: m_arr(arr), m_allocator(allocator), m_offset(arr->getSize()), m_ownerShip(o)
		{
			nullTerminate();
		}

...
HK_FORCE_INLINE void decode3( const T& vertex, hkVector4d* HK_RESTRICT vectorOut )
{
	enum
	{
...
template< typename T, int NUM_BITS_X, int NUM_BITS_Y >
HK_FORCE_INLINE void decodeFractions3( const T& vertex, hkVector4d* HK_RESTRICT vectorOut )
{
	enum
	{
		TOTAL_BITS	=	sizeof(T) << 3,
	...
	HK_ALIGN_DOUBLE( hkDouble64 ) invMask[4] = { 1.0f/MASK_X, 1.0f/hkDouble64(MASK_Y), 1.0f/hkDouble64(MASK_Z), 0.0f };

	hkVector4d	fractions; fractions.setMul( rv, (const hkVector4d&)invMask );
...
HK_FORCE_INLINE void	encode3(const hkAabb& aabb, hkVector4dParameter vertex, T* HK_RESTRICT output)
{
	...
	hkVector4d   clampled; clampled.setClamped(vertex, aabb.m_min, aabb.m_max);
	hkVector4d	delta; delta.setSub(aabb.m_max, aabb.m_min);
	hkVector4d	invDelta; invDelta.setReciprocal<HK_ACC_23_BIT,HK_DIV_IGNORE>(delta);
	HK_ASSERT( 0xf0345456, delta.greaterZero().allAreSet<hkVector4ComparisonMask::MASK_XYZ>());	// check for valid aabb
	hkVector4d	offset; offset.setSub(vertex, aabb.m_min);
	hkVector4d	fractions; fractions.setMul(offset, invDelta);

	HK_ALIGN_DOUBLE( hkDouble64 ) invMask[4] = { 1.0f/MASK_X, 1.0f/hkDouble64(MASK_Y), 1.0f/hkDouble64(MASK_Z), 0.0f };
	fractions.setAddMul( hkVector4d::getConstant<HK_QUADREAL_INV_2>(), fractions, (const hkVector4d&)invMask );

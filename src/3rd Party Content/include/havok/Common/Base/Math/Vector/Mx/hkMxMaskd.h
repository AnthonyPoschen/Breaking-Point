...
public:
	...
	template <int I> HK_FORCE_INLINE void set( hkVector4dComparisonParameter a );
	/// Get the I-th submask.
	template <int I> HK_FORCE_INLINE const hkVector4dComparison&	get() const;

	/// Set masks of self to the logical AND of respective masks in \a a and \a b.
	HK_FORCE_INLINE void setAnd(hkMxMaskParameterd a, hkMxMaskParameterd b);
	/// Set masks of self to the logical OR of respective masks in \a a and \a b.
	HK_FORCE_INLINE void setOr(hkMxMaskParameterd a, hkMxMaskParameterd b);
	...
	/// \warning [M:1-4 only] 
	HK_FORCE_INLINE void horizontalAnd4( hkVector4dComparison& andOut ) const;

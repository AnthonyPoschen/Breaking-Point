...
class hkDebugDisplay : public hkReferencedObject, public hkSingleton<hkDebugDisplay>
{
	...
	public:
		...
		void addGeometryInstance(hkUlong origianalGeomId, const hkTransform& transform, hkUlong id, int tag, hkUlong shapeIdHint);
			...
			/// Updates the 'isPickable' flag on an object id which has been added to the display
		void setGeometryPickable( hkBool isPickable, hkUlong id, int tag );

			/// Sets the color of an object that has already been added to all display viewer (targeted by the debug display).
			/// Please refer to the hkDisplayHandler class description for the meaning of id and tag.
		void setGeometryColor(hkColor::Argb color, hkUlong id, int tag);
		...
		void displayLineModelSpace(const hkQsTransform& worldFromModel, const hkVector4& start, const hkVector4& end, hkColor::Argb color, int id, int tag);

			/// Display a debug line using model coordinate endpoints.
			/// Please use the HK_DISPLAY_LINE_MODEL macro instead of calling this function directly.
		void displayLineModelSpace(const hkTransform& worldFromModel, const hkVector4& start, const hkVector4& end, hkColor::Argb color, int id, int tag);
		...
		void displayRay(const hkVector4& start, const hkVector4& direction, hkColor::Argb color, int id, int tag);
		...
		void displayRayModelSpace(const hkQsTransform& worldFromModel, const hkVector4& start, const hkVector4& direction, hkColor::Argb color, int id, int tag);

			/// Display a debug ray in model coordinates.
			/// Please use the HK_DISPLAY_RAY_MODEL macro instead of calling this function directly.
		void displayRayModelSpace(const hkTransform& worldFromModel, const hkVector4& start, const hkVector4& direction, hkColor::Argb color, int id, int tag);

			/// Display a debug arrow. Please use the HK_DISPLAY_ARROW macro
			/// instead of calling this function directly.
		void displayArrow(const hkVector4& start, const hkVector4& dir, hkColor::Argb color, int id, int tag);
		...
		void displayStarModelSpace(const hkQsTransform& worldFromModel, const hkVector4& position, hkReal scale, hkColor::Argb color, int id, int tag);

			/// Display a debug star in model coordinates.
			/// Please use the HK_DISPLAY_STAR_MODEL macro instead of calling this function directly.
		void displayStarModelSpace(const hkTransform& worldFromModel, const hkVector4& position, hkReal scale, hkColor::Argb color, int id, int tag);
		
			/// Display a debug plane. Please use the HK_DISPLAY_PLANE macro
			/// instead of calling this function directly.
		void displayPlane(const hkVector4& plane, const hkVector4& offset, hkReal scale, hkColor::Argb color, int id, int tag);
		
			/// Display a model space animation pose. Please use the HK_DISPLAY_MODEL_SPACE_POSE macro instead of
			/// calling this function directly.
		void displayModelSpacePose(int numTransforms, const hkInt16* parentIndices, const hkQsTransform* modelSpacePose, const hkQsTransform& worldFromModel, int id, int tag, hkColor::Argb color = 0x7fffffff);

			/// Display a local space animation pose. Please use the HK_DISPLAY_LOCAL_SPACE_POSE macro instead of
			/// calling this function directly.
		void displayLocalSpacePose(int numTransforms, const hkInt16* parentIndices, const hkQsTransform* localSpacePose, const hkQsTransform& worldFromModel, int id, int tag, hkColor::Argb color = 0x7fffffff);
		...
		void displayLitTriangle(const hkVector4& a, const hkVector4& b, const hkVector4& c, hkColor::Argb color);
...
#	define HK_DISPLAY_GEOMETRY_WITH_TRANSFORM(geometries, transform, color) hkDebugDisplay::getInstance().displayGeometry(geometries, transform, color, 0, 0)
#	define HK_DISPLAY_BOUNDING_BOX(aabb, color) hkDebugDisplay::getInstance().displayAabb(aabb, color, 0, 0)
#	define HK_DISPLAY_TRANSFORMED_BOUNDING_BOX(transform, aabb, color) hkDebugDisplay::getInstance().displayAabb(transform, aabb, color, 0, 0)

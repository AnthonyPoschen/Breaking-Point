...
public:
		...
		hkxScene();
		hkxScene(hkFinishLoadedObjectFlag f) : hkReferencedObject(f), m_modeller(f), m_asset(f), m_rootNode(f), m_selectionSets(f), m_cameras(f), m_lights(f), m_meshes(f), m_materials(f), m_inplaceTextures(f), m_externalTextures(f), m_skinBindings(f), m_splines(f) { }
		virtual ~hkxScene();
	...
	public:
      ...
      /// Original number of frames represented. May not be set for older exported scenes, so only valid if > 0.
		...
		hkResult getFullPathToNode (const hkxNode* theNode, hkArray<const hkxNode*>& pathOut) const;
			...
			/// Returns HK_FAILURE if the node doesn't belong to this scene
		hkResult getWorldFromNodeTransform (const hkxNode* theNode, hkMatrix4& worldFromNodeOut, int key=0) const;
};

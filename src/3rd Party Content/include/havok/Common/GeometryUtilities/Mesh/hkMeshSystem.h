...
class hkMeshSystem: public hkReferencedObject
{
	public:
		...
		virtual hkMeshShape* createCompoundShape(const hkMeshShape*const* shapes, const hkQTransform* transforms, int numShapes);
		...
		virtual hkMeshVertexBuffer* createVertexBuffer(const hkMeshVertexBuffer* templateVertexBuffer, int numVertices);
		...
		virtual void update() = 0;
			...
			/// Free the graphics resources maintained by this mesh sytem (but keep state information).

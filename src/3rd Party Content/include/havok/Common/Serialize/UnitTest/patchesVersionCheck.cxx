...
{
	const char* strings[] = {
#define	HK_PATCH_BEGIN(CLASSNAME, OLDVER, NEWCLASSNAME, NEWVER) NEWCLASSNAME,
...
#		undef		HK_PATCH_END
		...
		HK_NULL // Add a dummy entry in case we don't have any patch.
};
	...
	{
		const char* classname = strings[i];
		if (!classname)
			continue;

		bool prefixOK = hasPrefixes( prefixes, prefixCount, classname );

		if(!prefixOK) // check if we're supposed to skip it
		{
			for (int j=0; j<exceptionCount; j++)
			{
				if (hkString::strCmp( classname, exceptions[j]) == 0)
				{
					prefixOK = true;
				}
			}
		...
		HK_TEST2( prefixOK, "Patch for class " << classname << " is in wrong patches file.");
	}
}

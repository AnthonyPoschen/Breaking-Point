...
template<typename T>
inline hkResult HK_CALL hkSerializeUtil::save( const T* object, hkStreamWriter* writer, SaveOptions options )
{
	return save(object, T::staticClass(), writer, options);
}

template<typename T>
inline hkResult HK_CALL hkSerializeUtil::save( const T* object, const char* filename, SaveOptions options )
{
	return save(object, T::staticClass(), hkOstream(filename).getStreamWriter(), options);
}

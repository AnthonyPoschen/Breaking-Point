...
class hkaAnimationMapPoseJob : public hkaAnimationMappingJob
{
	...
	public:
		...
		/// Partition indices for the poseA
		hkInt16* m_partitionIndicesA;
		hkInt16 m_numPartitionsIndices;

		/// The number of transforms in the partition poses.
		/// This is only necessary when retargeting specific partitions
		/// as opposed to the full pose
		hkInt16 m_numPartitionTransformsA;
		hkInt16 m_numPartitionTransformsB;

		/// Whether the mapper should map the full pose or just the partition pose.

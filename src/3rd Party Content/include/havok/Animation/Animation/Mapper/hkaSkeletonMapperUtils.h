...
class hkaSkeletonMapperUtils
{
	public:
		...
		struct Params
		{
			...
			hkReal m_rotationMatchTolerance;
				...
				/// You can also force specific one-to-one mappings between partitions in case
				/// when partition names are different.
			hkArray<UserMapping> m_userPartitionMappingsAtoB;
			...
			hkArray<UserMapping> m_userMappingsBtoA;
				...
				/// If true, the system will auto detect partition mappings based on partition names.
			hkBool m_autodetectPartitionMap;
			...
			Params ( const Params& other) : 
				m_skeletonA(other.m_skeletonA),
				m_skeletonB(other.m_skeletonB), 
				...
				m_autodetectPartitionMap (other.m_autodetectPartitionMap),
				m_autodetectSimple (other.m_autodetectSimple), 
				m_autodetectChains (other.m_autodetectChains),
				m_mappingType(other.m_mappingType),
				m_motionParams(other.m_motionParams)
				{
					m_userPartitionMappingsAtoB = other.m_userPartitionMappingsAtoB;
			...
			/// Creates partition map (used by hkaSkeletonMapper) used to map partitions from skeleton A to skeleton B
			/// and vice-versa. For internal use only
		static bool HK_CALL createPartitionMapping(const Params& params, hkaSkeletonMapperData& aToB, hkaSkeletonMapperData& bToA);
	...
	private:
		...
		static bool HK_CALL checkRotation(	const Params& params, 
											const hkaSkeleton* skeletonA,
											const hkaSkeleton* skeletonB,
											hkInt16 boneA, 
											hkInt16 boneB, 
											const hkQsTransform& aToBTransform, 
											const hkQsTransform& bToATransform );

		static void HK_CALL addUserMappings(	const hkaSkeletonMapperUtils::Params& params, 
												const hkaSkeleton* skeletonA,
												const hkaSkeleton* skeletonB,
												const hkQsTransform* poseA, 
												const hkQsTransform* poseB,
												const hkArray<hkaSkeletonMapperUtils::UserMapping>& userMappingsAtoB,
												hkaSkeletonMapperData& aToB,
												hkArray<hkInt16>& map_BtoA);
};

...
class hkaSkeletonUtils
{
	public:
		...
		static hkBool HK_CALL getBoneChain(const hkaSkeleton& skeleton, hkInt16 startBone, hkInt16 endBone, hkArray<hkInt16>& bonesOut);
			...
			/// This method possibly allocates space in the array (since its length is unknown by the caller).
		static hkBool HK_CALL getBoneChainWithinPartition(const hkaSkeleton& skeleton, hkInt16 startBone, hkInt16 endBone, hkArray<hkInt16>& bonesOut);
		...
		static void HK_CALL getAncestors(const hkaSkeleton& skeleton, hkInt16 startPoint, hkArray<hkInt16>& bonesOut );
		...
		static hkBool HK_CALL hasValidPartitions( const hkaSkeleton& skeleton );

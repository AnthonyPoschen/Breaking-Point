...
class hkaAnimationControl : public hkReferencedObject
{
	public:
		...
		virtual void sampleTracks( hkQsTransform* transformTracksOut, hkReal* floatTracksOut) const;
		...
		/// Returns the partitions indices. May return null.
		virtual hkInt32 getNumPartitionIndices() const; 

			/// Returns the partitions indices. May return null.
		virtual const hkInt16* getPartitionIndices() const; 

			// Returns the skeleton mapper associated with this control or HK_NULL if there isn't one.
		virtual const hkaSkeletonMapper* getSkeletonMapper() const;
	...
	protected:
		...
		hkRefPtr<const hkaAnimationBinding> m_binding;

...
class hkaDefaultAnimatedReferenceFrame : public hkaAnimatedReferenceFrame
{
	public:
		...
		struct MotionExtractionOptions
		{
			...
			/// Constructs an empty reference frame
		hkaDefaultAnimatedReferenceFrame( hkaReferenceFrameTypeEnum frameType = REFERENCE_FRAME_DEFAULT ) : hkaAnimatedReferenceFrame( frameType ) {}
		...
		hkaDefaultAnimatedReferenceFrame( const MotionExtractionOptions& options, hkaReferenceFrameTypeEnum frameType = REFERENCE_FRAME_DEFAULT );
		...
		virtual void getReferenceFrame (hkReal time, hkQsTransform& motionOut) const HK_OVERRIDE;

			/// Returns the change in motion between two times.
			/// The default implementation assumes the animation is a looping animation.
		virtual void getDeltaReferenceFrame( hkReal time, hkReal nextTime, int loops, hkQsTransform& output, hkReal cropStartAmount, hkReal cropEndAmount ) const HK_OVERRIDE;

			/// Returns the length of time this reference frame is animated for.
		virtual hkReal getDuration() const HK_OVERRIDE;
	...
	public:
		...
		HK_FORCE_INLINE hkaDefaultAnimatedReferenceFrame( hkFinishLoadedObjectFlag flag, hkaReferenceFrameTypeEnum frameType = REFERENCE_FRAME_DEFAULT ) : hkaAnimatedReferenceFrame(flag, frameType), m_referenceFrameSamples(flag) {}

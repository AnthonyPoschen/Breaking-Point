...
class hkaMirroredSkeleton : public hkReferencedObject
{
	...
	public:
		...
		// returns the partitions from the original skeleton
		const hkaSkeleton::Partition* getPartitions() const;

		// returns the number of partitions in the original skeleton
		hkInt32 getNumPartitions() const; 
		...
		/// Allows the user to set all of the partition pairing information at once
		/// \param partitionPairMap The mapping of partitions to their mirrored partners
		void setPartitionPairMap( const hkInt16* partitionPairMap, int numPartitionPairMap );

		/// Allows the user to query partition pairing information
		/// \return The paired partition for pi
		/// \param pi Partition one to query pair information for
		hkInt16 getPartitionPair( hkInt16 pi ) const;
		...
		/// Computes partition pairing information from unique substrings of bone names
		/// \param ltag Array of strings uniquely identifying left bones
		/// \param ltag Array of strings uniquely identifying right bones
		void computePartitionPairingFromNames( const hkArrayBase< hkStringPtr >& ltag, const hkArrayBase< hkStringPtr >& rtag );

		/// Computes partition pairing information from unique substrings of bone names
		/// \param ltag Array of strings uniquely identifying left bones
		/// \param ltag Array of strings uniquely identifying right bones
		/// \param skeleton The skeleton that is being mirrored
		/// \param bonePairMap The buffer for the resultant partition map
		static void HK_CALL computePartitionPairingFromNames( 
			const hkArrayBase< hkStringPtr >& ltag, 
			const hkArrayBase< hkStringPtr >& rtag,
			const hkaSkeleton* skeleton,
			hkInt16* partitionPairMap );
	...
	private:
		...
		hkInt16* m_bonePairMap;
			...
			/// Partition pairing information
		hkInt16* m_partitionPairMap;
		...
		hkStringMap< const char* > m_annotationNameMap;
			...
			/// Partitions from the original skeleton.
			/// This pointer points to the values in the original skeleton.
			/// No new memory is allocated (motivated by data chunks).
		const hkaSkeleton::Partition* m_partitions;

			/// Number of partitions from the original skeleton
		hkInt16 m_numPartitions; // = m_skeleton->m_partitions.getSize()
